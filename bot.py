...
# ============================================================================
# 3. WebSocket Manager
#    Runs PumpPortal WSS in a daemon asyncio thread.
#    Parsed pool dicts are pushed into a thread-safe queue
#    consumed by the main sync loop.
# ============================================================================
...
class WebSocketManager:
    ...
    Subscriptions used:
      - subscribeNewToken   → new bonding curve launches (free, no key)
      - subscribeMigration  → tokens graduating to Raydium (free, no key)
    Response fields for new token (from PumpPortal docs):
      mint, name, symbol, marketCapSol, vSolInBondingCurve,
      initialBuy, txType, traderPublicKey, signature, bondingCurveKey
    """
    ...
    async def _connect_with_retry(self):
        """Reconnect with exponential backoff on disconnect/error."""
        backoff = config.wss_reconnect_delay
        while self.running:
            try:
                await self._connect()
                backoff = config.wss_reconnect_delay  # reset on clean exit
            except Exception as e:
                self.connected = False
    ...
    async def _connect(self):
        """
        Open one WebSocket connection and send all subscriptions on it.
        PumpPortal rule: ONE connection only — never open multiple.
        """
        ...
        async with websockets.connect(
            ...
            # Subscribe to Raydium migrations (graduating tokens)
    ...
    def _handle_message(self, raw: str):
        """Parse PumpPortal message and push to queue if it's a new token."""
        data = json.loads(raw)
        ...
        # Migration event (graduation to Raydium)
            ...
            if mint:
                ...
                # Optionally subscribe to trades on this token for price feed
                # (not implemented — use batch price fetch instead)

    def _parse_new_token(self, data: dict) -> dict | None:
        """
        Convert PumpPortal new-token event to our pool dict format.

        PumpPortal fields (from docs + observed responses):
          mint            - token contract address
          name            - token name
          symbol          - token symbol
          marketCapSol    - market cap in SOL
          vSolInBondingCurve - SOL locked in bonding curve = liquidity proxy
          initialBuy      - initial buy amount in tokens
          traderPublicKey - creator wallet
          signature       - tx signature
          bondingCurveKey - bonding curve account
        """
        mint = data.get("mint")
        if not mint:
            return None
        ...
        # Price: pump.fun has 1B token supply
        # marketCapSol = price_sol * 1_000_000_000
        ...
        # Volume estimate: use initialBuy converted to USD
        initial_buy_tokens = float(data.get("initialBuy", 0) or 0)
        vol_5m_usd     = initial_buy_tokens * price_usd  # rough 5m vol proxy
...
# ============================================================================
# 4. Detector (REST polling — fallback + BSC/BASE)
# ============================================================================
class Detector:
    ...
    def mark_seen(self, mint: str, chain: str = 'SOL'):
        """Called from main loop when a WSS pool is processed."""
    ...
    def get_new_pools(self, selected_chain):
        """
        REST-based new pool discovery.
        For SOL: acts as FALLBACK when WSS is active.
        For BSC/BASE: primary source (no WSS equivalent).
        """
        new_pools = []
        now       = time.time()
        pump_new  = 0

        # 1. Pump.fun REST (SOL) — skip if WSS is connected and working
        ...
        # 3. DexScreener token profiles (every 60s, all chains)
        ...
        # 4. Chain-specific DexScreener search (every 60s per chain)
...
class FilterEngine:
    ...
    def filter_and_score(self, pool):
        ...
        skip_age    = (source in ('pumpfun', 'pumpfun_rest', 'pumpportal_wss') or
...
def run_bot():
    ...
    # ── WebSocket setup ──────────────────────────────────────────────────────
    wss_queue = queue.Queue(maxsize=500)   # thread-safe bridge WSS → main loop
    ...
    while True:
        ...
        while not wss_queue.empty():
            try:
                ...
                if pool['mint'] not in detector.seen_mints:
                    # Mark seen so REST fallback doesn't duplicate it
        ...
        # WSS pools go first (they're faster/more current)
        ...
        for pool in all_candidates:
            ...
            # Lower threshold for pump.fun/WSS sources (no momentum data)
            if filtered.get('source') in ('pumpfun', 'pumpfun_rest', 'pumpportal_wss'):
...
class Detector:
    ...
    def _trim_per_chain(self):
        for chain in list(self.seen_mints_per_chain.keys()):
            if len(self.seen_mints_per_chain[chain]) > self.max_per_chain:
                self.seen_mints_per_chain[chain] = set(list(self.seen_mints_per_chain[chain])[-self.max_per_chain:])
            ...
            try:
                ...
                if resp and resp.status_code == 200:
                    ...
                    for item in data:
                        ...
                        pair_data = self._fetch_pair_info(mint, chain_id)
                        if pair_data:
                            created_ts = pair_data.get('pairCreatedAt', now * 1000)
                            if created_ts > 1e12:
                                created_ts /= 1000
                            created_at = datetime.datetime.fromtimestamp(created_ts)
                            pool = {
    ...
    def get_momentum_pools(self, selected_chain, config: BotConfig):
        ...
        # 2. DexScreener dedicated momentum search (per chain, rate limited)
        chains_to_scan = ['SOL'] if selected_chain == 'SOL' else \
                         ['BSC'] if selected_chain == 'BSC' else \
...
class FilterEngine:
    ...
    def filter_and_score(self, pool):
        source = pool.get('source', '')
        # Only skip filters for exact pumpfun source or any momentum source
        skip_filters = (source == 'pumpfun' or source.endswith('_momentum'))
        if not skip_filters:
            try:
                age = (datetime.datetime.now() - pool['created_at']).total_seconds()
                if age < self.config.age_min or age > self.config.age_max:
...
def run_bot():
    ...
    while True:
        ...
        daily_loss = reporter.daily_summary.get('net_profit', 0)
        can_buy_now = (daily_loss >= config.daily_loss_limit and
                      (buy_pause_until is None or datetime.datetime.now() >= buy_pause_until))
        ...
        for pool in all_candidates:
            ...
            reporter.log_detection(filtered)
            notifier.send(f"📈 BUY SIGNAL [{filtered['chain']}] {filtered['symbol']} ({filtered['name']})\n"
                         f"📋 CA: <code>{filtered['mint']}</code>\n"
        ...
        wins = sum(1 for t in full if t['profit_usd'] > 0)
        wr = (wins / len(full) * 100) if full else 0
        logger.info(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Balance: ${simulator.balance:.2f} | Trades: {len(full)} | WR: {wr:.0f}% | Open: {len(simulator.positions)}")
        time.sleep(15)
